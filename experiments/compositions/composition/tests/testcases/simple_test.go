...
func DontTestSimpleExpansionJob(t *testing.T) {
	//t.Parallel()
...
// We used to test with expander.name set to none and then set it to a non-empty value
// But we recently made changes that made it a required field. So we cannot start with an expander with missing name anymore.
func DontTestSimpleCompositionStatusValidation(t *testing.T) {
	//t.Parallel()
	s := scenario.NewBasic(t)
	defer s.Cleanup()
	s.Setup()
...
func TestCompositionWithSimpleSchema(t *testing.T) {
	//t.Parallel()
	s := scenario.NewBasic(t)
	defer s.Cleanup()
	s.Setup()
